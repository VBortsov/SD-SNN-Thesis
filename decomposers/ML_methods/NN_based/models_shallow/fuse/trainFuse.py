...
from decomposers.ML_methods.NN_based.models_shallow.common import DepthExpansion1D, ThreeParallelShallowBranches
...
class TrainConfig:
    ...
    output_dir: str = "decomposers/ML_methods/NN_based/saved_models/fuse_shallow"
...
class ThreeParallelWithFuseDecomposer(nn.Module):
    """Shallow decomposer using parallel branches + reusable Fuse stage."""
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        self.fuse = Fuse(
            in_channels=3 * cfg.branch_channels,
            out_channels=cfg.fused_channels,
            norm=cfg.norm,
            activation=cfg.activation,
            dropout=cfg.dropout,
            num_groups=cfg.num_groups,
        ...
        self.depth_expansion = DepthExpansion1D(
            in_channels=cfg.fused_channels,
            extra_conv_layers=cfg.extra_conv_layers,
            extra_conv_channels=(cfg.extra_conv_channels or None),
            kernel_size=cfg.extra_conv_kernel_size,
            dilation=cfg.extra_conv_dilation,
            activation=cfg.extra_conv_activation,
            norm=cfg.extra_conv_norm,
            dropout=cfg.extra_conv_dropout,
            num_groups=cfg.extra_conv_num_groups,
            residual=cfg.extra_conv_residual,
        )
        self.head = nn.Conv1d(self.depth_expansion.output_channels, cfg.out_channels, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Input tensor.
        """
        fused = self.depth_expansion(self.fuse(self.features(x)))
        return self.head(fused)


def parse_args() -> TrainConfig:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Train shallow decomposer with Fuse stage.")
    ...
    parser.add_argument("--extra-conv-dropout", type=float, default=0.0)
    ...
    parser.add_argument("--output-dir", type=str, default="decomposers/ML_methods/NN_based/saved_models/fuse_shallow")
...
def create_model(cfg: TrainConfig, device: torch.device) -> nn.Module:
    ...
    """
    return ThreeParallelWithFuseDecomposer(cfg=cfg).to(device)
